...
const Navbar = ({ openNav }: Props) => {
 ...
 const stickyStyle = navSticky ? "md:hidden hidden md:flex flex-row shadow-gray-100 shadow-sm":""
    ...
    <header className="text-gray-400  bg-gray-900 body-font">
      <div className="container mx-auto ${stickyStyle}  flex flex-wrap p-5 flex-col md:flex-row items-center  backdrop-blur-md z-[50]">
        <a className="flex title-font font-medium items-center text-white mb-4 md:mb-0 ">
          ...
          />
          <span className="ml-2 text-xl font-serif font-medium">Hafsa Kamali</span>
        </a>
  ...
  <li> <Link href="/contact" className="mr-5 text-white  px-4 hover:scale-110  z-20 hover:underline hover:underline-offset-4">Contact</Link></li>
  </ul>
  </nav>

<button className="md:inline-block items-centers hidden flex-row gap-4 bg-gradient-to-r from-slate-500 to-gray-700 border border-white py-2 px-3 focus:outline-none hover:from-slate-500 to-gray-500 hover:scale-110  text-white rounded-md text-base mt-4 md:mt-0">
  Log In
</button>
